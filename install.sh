...
if [ -z "$PYTHON" ]; then
  echo "✗ Python 3.10+ is required. Install with: brew install python@3.12"
  exit 1
fi
...
if [ "$PY_MAJOR" -lt 3 ] || { [ "$PY_MAJOR" -eq 3 ] && [ "$PY_MINOR" -lt 10 ]; }; then
  echo "✗ Python $PY_VER too old. Need 3.10+. Install with: brew install python@3.12"
  exit 1
fi
echo "✓ Python $PY_VER"
...
"$VENV/bin/python3" "$PLUGIN_DIR/scripts/generate.py" "$LANG_CODE"
echo "✓ Audio ready"
...
"$VENV/bin/python3" "$PLUGIN_DIR/scripts/patch-settings.py" "$PLUGIN_DIR" "$SETTINGS"
